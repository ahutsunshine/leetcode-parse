...
import com.leetcode.model.response.Error;
...
public class HttpUtil {
    ...
    public static APIResponse getErrorIfFailed(String res) {
        ...
        if (isErrorMessage(msg)) {
            List<Error> errors = msg.getErrors();
            String info = CollectionUtils.isEmpty(errors) ?
                    "Request failed" : msg.getErrors().get(0).getMessage();
